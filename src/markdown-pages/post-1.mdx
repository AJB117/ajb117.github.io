...
Javascript-driven features to blog posts (e.g. visualization with D3, brief interactive demos, in-browser inference, etc.)
...
for better interactivity within Markdown posts. To show it off, here's a <!-- markdownlint-capture --> todo "app" within this post's Markdown content:
...
A couple of things: don't try to get `remark-html-katex` working (it won't), and don't forget to import
the katex stylesheet (`katex/dist/katex.min.css`) in the template you're using to serve your MDX.
Now I can write math in my MDX! A nice, quick fact:
